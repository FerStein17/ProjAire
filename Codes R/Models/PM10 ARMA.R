...
datapm10<-ts.intersect(PM10,time,time2,dd1,dd2,dd3,dd4,dd5,dd6,dd7,covid)

regpm10<-lm(PM10~time+time2+dd2+dd3+dd4+dd5+dd6+dd7+covid,data=datapm10)
summary.lm(regpm10,robust=TRUE)

#wald
waldtest(regpm10,reg2)

# Sí hay estacionalidad

# ¿White Noise? NO
summary.lm(regpm10,robust=TRUE)
resid<-residuals(regpm10)
...
#el mejor es el garch70
garch70 <- garchFit(residualesarma~garch(7,0), data = residualesarma)
summary(garch70)
predictse<-predict(garch70)
predictse

modelo<-garchFit(resid~arma(2,1)+garch(7,0), data=resid)
summary(modelo)
...
modelo<-garchFit(resid~arma(2,1)+garch(7,4), data=residualesarma)
summary(modelo)
